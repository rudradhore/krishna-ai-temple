      ...
      <header className="flex-none z-50 bg-white/90 backdrop-blur-md border-b border-yellow-200 p-3 shadow-sm flex justify-between items-center sticky top-0">
        ...
        <div className="flex gap-2">
            <button 
                ...
                className={`px-3 py-1 rounded-full text-xs font-bold border shadow-sm ${mode === 'japa' ? 'bg-orange-100 text-orange-900 border-orange-300' : 'bg-white text-gray-900 border-gray-300'}`}
            >
                {mode === 'chat' ? 'Japa Mode' : 'Chat'}
            </button>
            <button onClick={toggleAudio} className={`p-2 rounded-full border shadow-sm ${isAudioEnabled ? 'bg-yellow-100 text-yellow-900 border-yellow-300' : 'bg-gray-100 text-gray-500 border-gray-300'}`}>
      ...
      {mode === 'japa' ? (
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-6 animate-in fade-in zoom-in duration-500">
            ...
            <div className="flex gap-6 items-center">
                <button 
                    onClick={toggleMic}
                    className={`w-20 h-20 rounded-full flex items-center justify-center shadow-xl transition-all active:scale-95 ${isListening ? 'bg-red-600 text-white ring-4 ring-red-200 animate-pulse' : 'bg-gray-200 text-gray-700 border border-gray-300'}`}
                >
